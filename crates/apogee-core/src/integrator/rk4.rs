...
pub struct Rk4 {
    ...
    pub step: Seconds<f64>,
}

impl Rk4 {
    /// Create a new RK4 integrator with the given fixed step.
    pub fn new(step: Seconds<f64>) -> Self {
        assert!(step.into_value() > 0.0, "RK4 step must be positive");
        Self { step }
    }
}
...
impl Integrator for Rk4 {
    fn step(
        ...
        let step_value = self.step.into_value();
        let sign = if dt_value >= 0.0 { 1.0 } else { -1.0 };
        let h = step_value * sign;
        let n = (dt_value.abs() / step_value).ceil() as usize;
        if n == 0 {
            return IntegrationResult {
                accepted: true,
                error_estimate: 0.0,
                step_taken: Seconds::new(0.0),
            };
        }
        let mut s = state.clone();
        let mut total = 0.0_f64;
        for _ in 0..n {
            let remaining = dt_value - total;
            let step = if remaining.abs() < step_value {
                remaining
            } else {
                h
            };
...
fn rk4_stage(
    ...
    s2.attitude = integrate_attitude(&s2.attitude, &s2.angular_velocity, Seconds::new(0.5 * h));
    let k2 = derivative_fn(&s2);

    let mut s3 = state.clone();
    s3.position += k2.velocity * (0.5 * h);
    s3.velocity += k2.acceleration * (0.5 * h);
    s3.attitude = integrate_attitude(&s3.attitude, &s3.angular_velocity, Seconds::new(0.5 * h));
    let k3 = derivative_fn(&s3);

    let mut s4 = state.clone();
    s4.position += k3.velocity * h;
    s4.velocity += k3.acceleration * h;
    s4.attitude = integrate_attitude(&s4.attitude, &s4.angular_velocity, Seconds::new(h));
    ...
    state.velocity +=
        (k1.acceleration + 2.0 * k2.acceleration + 2.0 * k3.acceleration + k4.acceleration)
            * (h / 6.0);
    state.attitude = integrate_attitude(&state.attitude, &state.angular_velocity, Seconds::new(h));
...
mod tests {
    ...
    fn test_rk4_harmonic_oscillator() {
        ...
        let mut integrator = Rk4::new(Seconds::new(0.01));
        integrator.step(
            &mut state,
            &|s: &StateVector| {
            ...
            Seconds::new(2.0 * std::f64::consts::PI),
        );
