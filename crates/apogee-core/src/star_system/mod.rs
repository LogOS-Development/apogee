...
impl CelestialBody {
    ...
    pub fn direction_to(&self, origin: &PositionVec) -> PositionVec {
        PositionVec::new((origin.value() - self.position.value()).normalize())
    }

    /// Displacement vector from this body to an observer at `origin` (meters).
    /// Convention: vector points from self to the observer.
    pub fn vector_to(&self, origin: &PositionVec) -> PositionVec {
        PositionVec::new(origin.value() - self.position.value())
    }

    /// Scalar distance from this body to an observer at `origin` (meters).
    pub fn distance_to(&self, origin: &PositionVec) -> f64 {
...
impl StarSystem {
    ...
    pub fn at_epoch(epoch: Epoch) -> Self {
        ...
        // Phase 1: SSB ≈ Sun.
        let sun_position_m = PositionVec::new(nalgebra::Vector3::zeros());

        // Earth is opposite the Sun direction at distance r_au.
        let earth_position_m = PositionVec::new(-sun_direction * r_au * AU);
        ...
        .normalize();
        let earth_rotation_axis = PositionVec::new(earth_pole);
        ...
        bodies.insert(
            ...
            CelestialBody {
                ...
                velocity: VelocityVec::default(),
                acceleration: AccelerationVec::default(),
                rotation_axis: PositionVec::new(nalgebra::Vector3::z()),
                rotation_angle: 0.0,
            },
        );
        bodies.insert(
    ...
    pub fn vector_between(&self, observer_name: &str, target_name: &str) -> Option<PositionVec> {
    ...
    /// Convention: vector points from target to observer.
    pub fn direction_between(&self, observer_name: &str, target_name: &str) -> Option<PositionVec> {
        let observer = self.body(observer_name)?;
        let target = self.body(target_name)?;
