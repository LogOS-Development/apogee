...
where
    T: fmt::Display + Clone + nalgebra::Scalar + PartialEq + std::fmt::Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
mod tests {
    ...
    fn scalar_times_vector() {
        ...
        let v = VectorQuantity::<f64, 3, dim::Dimensionless>::new(nalgebra::Vector3::new(
            1.0, 2.0, 3.0,
        ));
        let p = s * v;
    ...
    #[test]
    fn tensor_sub() {
        let a: TensorQuantity<f64, 2, 2, dim::Dimensionless> =
            TensorQuantity::new(nalgebra::SMatrix::from_element(5.0));
        let b: TensorQuantity<f64, 2, 2, dim::Dimensionless> =
